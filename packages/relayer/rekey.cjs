...
(async () => {
  ...
  const oldPw = (await ask(`Enter OLD keystore password for ${inPath}: `)).trim();
  ...
  const newPw1 = (await ask(`Enter NEW password to encrypt ${outPath}: `)).trim();
  if (!newPw1) throw new Error('New password empty');
  const newPw2 = (await ask(`Re-enter NEW password: `)).trim();
  if (newPw1 !== newPw2) throw new Error('New passwords do not match');

  const v3 = await wallet.toV3String(newPw1, { kdf: 'scrypt' }); // strong default
